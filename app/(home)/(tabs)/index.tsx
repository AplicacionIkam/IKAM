...
const App = () => {
    ...
    const obtenerPymesPorCategoria = useCallback(async (nombreCategoria: string) => {
        try {
            const q = query(
                collection(db, "pyme"),
    ...
    useEffect(() => {
        if (busquedaCategoria) {
            const categoriasFiltradas = categorias.filter(categoria =>
                categoria.nombreCat.toLowerCase().includes(busquedaCategoria.toLowerCase())
            );
    ...
    }, [categoriaSeleccionada, categorias, obtenerPymesPorCategoria, obtenerTodasLasPymes]);
